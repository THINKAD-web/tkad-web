...
export async function POST(req: Request) {
  try {
    ...
    if (!limiter.check(`google-complete:${ip}`)) {
    ...
    if (!pending) {
      return apiError("GOOGLE_PENDING_EXPIRED", 400, {
    ...
    void seedSignupOnboardingPreference(created.userId, signupRole).catch(
      (err) => console.error("[auth/google/complete] onboarding preference", err),
    );
    ...
    return apiServerError(e, "auth/google/complete");
  }
}
