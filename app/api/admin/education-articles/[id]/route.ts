...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  return json({ educationArticle: row });
}
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const db = getPrisma();
  const existing = await db.educationArticle.findUnique({ where: { id } });
  ...
  const educationArticle = await db.educationArticle.update({
    where: { id },
    data,
  });
  return json({ educationArticle });
}
