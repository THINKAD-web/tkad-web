...
export default function AdminCommunityPage() {
  ...
  return (
    <div className="space-y-6">
      <header className="border-2 border-bx-black bg-bx-black p-6 text-bx-white">
        ...
        <p className="mt-2 font-mono text-[11px] tracking-tight text-bx-white/65">
      ...
      <div className="flex flex-wrap gap-0">
        <button
          ...
          className={`-ml-[2px] inline-flex items-center gap-2 border-2 border-bx-black px-5 py-2.5 font-mono text-[11px] font-bold uppercase tracking-[0.22em] transition-colors ${
            tab === "posts"
      ...
      {/* 필터 */}
      <div className="flex flex-wrap items-center gap-3 border-2 border-bx-black bg-bx-off p-4">
            ...
            {(["all", "published", "hidden", "deleted"] as StatusFilter[]).map(
              (s) => (
                <button
                  ...
                  className={`-ml-[2px] border-2 border-bx-black px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.18em] transition-colors ${
                    statusFilter === s
        ...
        <label className="ml-auto inline-flex items-center gap-2 font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-bx-black">
          <input
            type="checkbox"
            checked={reportsOnly}
        ...
        <button
          ...
          onClick={() => void load()}
          disabled={loading}
          className="inline-flex items-center gap-1.5 border-2 border-bx-black bg-bx-white px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-bx-black transition-colors hover:bg-bx-black hover:text-bx-white disabled:opacity-50"
        >
          {loading ? (
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    published: { label: "정상", cls: "border-bx-black bg-bx-white text-bx-black" },
    hidden: { label: "숨김", cls: "border-bx-accent bg-bx-accent text-bx-white" },
    deleted: { label: "삭제", cls: "border-bx-black bg-bx-black text-bx-white" },
  };
    ...
    <div className="flex flex-wrap gap-1.5">
      {reportCount > 0 ? (
        <button
          ...
          className="inline-flex items-center gap-1 border-2 border-bx-accent bg-bx-accent px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-bx-white hover:bg-bx-black hover:border-bx-black"
          title="신고 사유 보기"
        >
      ...
      {status === "hidden" || status === "deleted" ? (
        <button
          ...
          className="inline-flex items-center gap-1 border-2 border-bx-black bg-bx-white px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-bx-black hover:bg-bx-black hover:text-bx-white"
          title="복구"
        >
      ...
      <button
        ...
        className="inline-flex items-center gap-1 border-2 border-bx-accent bg-bx-white px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-bx-accent hover:bg-bx-accent hover:text-bx-white"
...
function PostsTable({
  ...
  if (loading && rows.length === 0) {
    return <p className="font-mono text-[12px] uppercase tracking-[0.22em] text-bx-gray-dim">{`// `}로딩 중…</p>;
  }
  if (rows.length === 0) {
    return (
      <div className="border-2 border-bx-black bg-bx-off p-12 text-center font-mono text-[12px] uppercase tracking-[0.22em] text-bx-gray-dim">
        {`// `}조건에 맞는 게시글이 없습니다.
      </div>
    );
  }
  return (
    <div className="overflow-x-auto border-2 border-bx-black">
      <table className="w-full text-sm">
        <thead className="bg-bx-black text-bx-accent">
          <tr>
            {["상태", "카테고리", "제목 / 발췌", "작성자", "신고", "통계", "작성", "관리"].map(
              (h) => (
                <th
                  key={h}
                  className="px-3 py-2.5 text-left font-mono text-[10px] font-bold uppercase tracking-[0.22em] border-r-2 border-bx-white last:border-r-0"
                >
                  {h}
                </th>
              ),
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-t-2 border-bx-black">
              ...
              <td className="px-3 py-2.5 align-top">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block font-bold text-bx-black hover:text-bx-accent"
                >
                  {r.title}
                </a>
                <p className="mt-1 line-clamp-2 font-mono text-[11px] tracking-tight text-bx-gray-dim">
                  {r.bodyExcerpt}
                </p>
              </td>
              <td className="px-3 py-2.5 align-top">
                <p className="font-mono text-[11px] text-bx-black">
                  {r.isAnonymous ? "익명" : r.authorName}
                </p>
                {r.authorIp ? (
                  <p className="mt-0.5 font-mono text-[10px] text-bx-gray-dim">
                    IP {r.authorIp}
                  </p>
                ) : null}
              </td>
              <td className="px-3 py-2.5 align-top text-center">
                <span
                  className={`font-mono text-sm font-bold tabular-nums ${
                    r.reportCount > 0 ? "text-bx-accent" : "text-bx-gray-dim"
                  }`}
                >
                  {r.reportCount}
                </span>
              </td>
              <td className="px-3 py-2.5 align-top font-mono text-[10px] uppercase tracking-[0.18em] text-bx-gray-dim">
              ...
              <td className="px-3 py-2.5 align-top font-mono text-[11px] tabular-nums text-bx-gray-dim">
                {fmt(r.createdAt)}
              </td>
              <td className="px-3 py-2.5 align-top">
                <ActionButtons
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-t-2 border-bx-black">
              ...
              <td className="px-3 py-2.5 align-top">
                <p className="line-clamp-3 text-sm text-bx-black">{r.body}</p>
              </td>
              <td className="px-3 py-2.5 align-top">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="line-clamp-2 font-mono text-[11px] tracking-tight text-bx-accent hover:text-bx-black"
                >
                  {r.postTitle}
                </a>
              </td>
...
function ReportsModal({
  ...
  return (
    ...
    >
      <div className="flex max-h-[80vh] w-full max-w-2xl flex-col border-2 border-bx-black bg-bx-white">
          ...
          {loading ? (
            ...
            <ul className="space-y-0">
              {reports.map((r) => (
                <li
                  key={r.id}
                  className="-mt-[2px] border-2 border-bx-black bg-bx-white p-4"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2 font-mono text-[10px] uppercase tracking-[0.22em] text-bx-gray-dim">
                  ...
                  <p className="mt-2 whitespace-pre-wrap text-sm text-bx-black">
                    {r.reason}
                  </p>
                </li>
              ))}
