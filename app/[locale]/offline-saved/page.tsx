...
export default function OfflineSavedMediaPage() {
  ...
  return (
    <HomeLandingDayNight>
      <div className="tkad-landing-neon mx-auto max-w-lg px-4 py-10 text-white">
          ...
          <ul className="mt-6 space-y-3">
            {items.map((m) => (
              <li key={m.id}>
                <Link
                  href={m.pagePath}
                  className="flex gap-3 rounded-2xl border border-white/12 bg-white/5 p-3 hover:border-cyan-400/30"
                >
                  {m.imageUrl ? (
