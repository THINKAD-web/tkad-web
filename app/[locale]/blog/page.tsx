  ...
  return (
    <>
      ...
      <section className="bg-bx-off py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {/* Filters */}
          <div className="flex flex-col gap-4 border-2 border-bx-black bg-bx-white p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-0">
              <span className="mr-2 flex items-center gap-1 font-mono text-[11px] font-bold uppercase tracking-[0.22em] text-bx-accent">
          ...
          {/* Result count */}
          <div className="mt-4 flex items-center justify-between font-mono text-[11px] uppercase tracking-[0.18em] text-bx-gray-dim">
            <div className="flex items-center gap-2">
              <Tag className="h-3.5 w-3.5 text-bx-accent" />
              <span>
            ...
            {filteredPosts.map((post) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      "relative h-40 border-b-2 border-bx-black",
                      post.thumbnailColor,
                    )}
                  >
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Sparkles className={cn("h-10 w-10", post.thumbnailColor === "bg-bx-off" ? "text-bx-accent" : "text-bx-white")} />
                    </div>
                    <div className="absolute left-3 top-3 flex items-center gap-2">
                      {categoryLabel && (
                        <span className="border-2 border-bx-white bg-bx-black px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-bx-white">
                          [ {categoryLabel} ]
                        </span>
                      )}
                    </div>
                    <div className="absolute bottom-3 left-3 flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-bx-white">
                      <Calendar className="h-3.5 w-3.5" />
                      <span>{formattedDate}</span>
                    </div>
                  </div>
                  <header className="border-b-2 border-bx-black p-4">
                    <h3 className="text-sm font-bold tracking-tight text-bx-black sm:text-base">
                      {title}
                    </h3>
                  </header>
          ...
          {filteredPosts.length === 0 && (
            <div className="mt-10 border-2 border-bx-black bg-bx-white px-6 py-10 text-center font-mono text-sm text-bx-gray-dim">
              {`// `}{isKo
