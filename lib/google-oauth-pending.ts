...
export const GOOGLE_PENDING_COOKIE = "tkad_google_oauth_pending";
const MAX_AGE_SEC = 60 * 15;

export type { KakaoSignupRole as GoogleSignupRole } from "@/lib/kakao-oauth-pending";
export {
...
function pendingSecret(): string | null {
  ...
  if (process.env.NODE_ENV !== "production") {
    return "dev-google-pending-secret";
  }
  return null;
}
...
function encodePending(data: GooglePendingPayload): string | null {
...
function decodePending(token: string): GooglePendingPayload | null {
  const secret = pendingSecret();
  if (!secret) return null;
  ...
  let data: GooglePendingPayload;
  try {
    data = JSON.parse(payload) as GooglePendingPayload;
  } catch {
    return null;
  }
...
export async function setGooglePending(
  data: Omit<GooglePendingPayload, "v" | "exp">,
): Promise<boolean> {
...
export async function readGooglePending(): Promise<GooglePendingPayload | null> {
  const cookieStore = await cookies();
  const raw = cookieStore.get(GOOGLE_PENDING_COOKIE)?.value;
  if (!raw) return null;
