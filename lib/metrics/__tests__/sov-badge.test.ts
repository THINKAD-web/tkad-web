...
test("V-3: loop 매체(DOOH·옥내 사이니지)에 SOV 배지가 붙는다", () => {
  assert.equal(
    needsSovBadge({ type: "dooh", mediaSubCategory: "digital_signage" }),
    true,
  );
  assert.equal(needsSovBadge({ type: "dooh", mediaSubCategory: "led_screen" }), true);
  assert.equal(needsSovBadge({ type: "dooh", mediaSubCategory: "elevator" }), true);
...
test("V-3: subCategory / mediaSubCategory 양쪽 필드명을 받는다", () => {
  assert.equal(needsSovBadge({ type: "dooh", subCategory: "digital_signage" }), true);
