...
## 부록 — R-02 가격 정정 감사 추적 공백 (2026-09-08 추가)

조사 과정에서 `execute-r02-price-x10.mts`(scripts/execute-flagged-a-group-batch1/2와 함께 PR1 마지막 라운드에서
고친 4개 스크립트 중 하나)가 이 조사가 참조하는 "552건 CPM 정정"과 사실상 같은 사건임을 확인했다.
이 부록은 캐시 원인 조사와는 별개의 발견이지만, **같은 근본 패턴**(사람이 파이프라인 중간 단계를 빠뜨려도
아무도 눈치채지 못하는 구조)이라 함께 기록한다.

### 타임라인 (GitHub PR/커밋 기준 — 로컬 git log는 squash되어 있어 신뢰 불가)

- **2026-08-31T00:30:11Z** — PR #504 병합. `scripts/dry-run-r02-price-x10.mts`로 552건 dry-run(423 approve /
  128 reject / 1 review) 생성. PR 본문에 명시: **"R-02 실제 price 수정은 dry-run 리포트 승인 후 별도 지시"**.
  같은 PR에서 `execute-r02-price-x10.mts`의 트랜잭션 타임아웃도 고쳐둠(423건을 50건 단위로 커밋) — 즉 실행
  준비는 끝나 있었음.
- 이후 이 레포의 어떤 PR·커밋·이슈·댓글에도 R-02 실행에 대한 언급이 전혀 없음(GitHub 커밋/PR/코드 검색
  전수 확인). `writeBatchExecuteReport`가 실제 `--execute` 실행 시 반드시 남기는
  `reports/r02-price-x10-execute.json`도 이 레포 역사 어디에도 존재한 적이 없음.
- **2026-09-08** — 관리자 페이지에서 합정역 지하철 6호선 CM보드(`cmqste6hy00000akoxccqnmi5`) 가격을 직접
  확인한 결과 **₩28,000,000(post-fix)** — 즉 **실행은 실제로 일어났음**. 이 레포가 추적하지 않는 경로(로컬
  머신에서 직접 실행, 또는 리포트 파일을 커밋하지 않은 실행)로 반영된 것으로 추정.

### 결론

**"423건 승인 → 실행 여부가 레포에 기록되지 않음 → 아무도 확인하지 않음"** — 이 흐름 자체가 이번 3개월간
지속된 ISR Writes 비용 조사의 원인들과 같은 종류의 문제다. 코드가 맞고 틀리고의 문제가 아니라, **여러 단계로
이루어진 작업에서 마지막 단계(실행 완료 기록, 또는 이번 캐시 조사에서는 무효화 호출)가 사람의 기억에
의존할 때 반복적으로 빠진다**는 패턴.

### 권장 (지금 당장 할 필요는 없음 — 백로그)

대량 데이터 정정 작업을 (a) dry-run 승인 → (b) 실제 실행 → (c) 실행 리포트 커밋 → (d) 캐시 무효화까지
**하나의 스크립트/파이프라인으로 묶어서 사람이 단계 사이에 빠뜨릴 수 없게 만들 것.** 이번 조사에서
one-off 스크립트 마지막 줄에 `revalidateMediaCachesAfterScript()`를 박아넣은 것과 정확히 같은 원칙 —
"사람이 기억해서 실행하는 별도 단계"는 결국 빠진다. `reports/backlog-root-price-field-ssot.md`처럼 이런
종류의 후속 작업을 명시적으로 추적하는 백로그 항목으로 등록해 둘 것을 권장한다("대량 데이터 정정
파이프라인 표준화").

### 참고 스크립트 (이번 라운드에서 작성, 읽기 전용/DB 불필요 분리)

- `scripts/audit-r02-price-x10-current-state.mts` — 423건 전부 현재 DB 상태 대조(A/B/C/missing 분류). 읽기
  전용, DB 접근 가능한 환경에서 실행.
- `scripts/revalidate-r02-price-x10-cache.mts` — 위 스크립트의 출력을 받아 그룹별 캐시 무효화. DB 접근
  불필요(HTTP만).
