...
export function HomeCommunityNewsletter({ isKo, className }: Props) {
  ...
  return (
    <div
      className={cn(
        "tkad-neon-border tkad-neon-glow relative overflow-hidden rounded-[28px] border border-white/12 bg-white/[0.04] p-5 text-white backdrop-blur sm:p-6",
        className,
      )}
    >
      <div
      ...
      <div className="relative">
        ...
        <h3 className="mt-2 font-sans text-lg font-bold tracking-tight text-white sm:text-xl">
          {isKo ? (
            <>
        ...
        <form onSubmit={onSubmit} className="mt-4 flex flex-col gap-3 sm:flex-row">
          ...
          <input
            ...
            className="h-11 min-w-0 flex-1 rounded-full border border-white/14 bg-black/35 px-4 font-mono text-sm font-semibold text-white placeholder:text-white/45 outline-none backdrop-blur transition-colors focus:border-white/22 focus:ring-2 focus:ring-[#a855f7]/30 disabled:cursor-not-allowed disabled:opacity-60"
          />
          <button
            type="submit"
            disabled={status === "loading" || status === "success"}
            className="h-11 shrink-0 rounded-full border border-white/22 bg-[linear-gradient(135deg,rgba(168,85,247,0.95),rgba(34,211,238,0.95),rgba(236,72,153,0.95))] px-5 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-white shadow-[0_14px_44px_rgba(0,0,0,0.5)] transition-transform hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {status === "loading"
              ? isKo
                ? "전송 중…"
