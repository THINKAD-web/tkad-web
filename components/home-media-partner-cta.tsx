 ...
 * 다크 글래스 + 딥틸/시안/핑크 그라데이션 라이닝 + 라이트 모드에서도 가독성 유지.
 */
export function HomeMediaPartnerCta({ isKo }: { isKo: boolean }) {
  return (
    <section
      className="relative isolate overflow-hidden border-y border-gray-200 bg-gradient-to-r from-hermes/10 via-white to-cyan-50/30 text-gray-900 dark:border-white/10 dark:bg-[#05050a] dark:text-white"
    ...
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-y-12 left-1/2 -z-0 hidden h-[200%] w-[120%] -translate-x-1/2 bg-[radial-gradient(circle_at_30%_50%,rgba(15,95,92,0.32),transparent_55%),radial-gradient(circle_at_75%_50%,rgba(34,211,238,0.28),transparent_55%),radial-gradient(circle_at_50%_50%,rgba(236,72,153,0.18),transparent_60%)] dark:block"
      />
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-5">
        ...
        <Link
          href="/register/media"
          className="group inline-flex shrink-0 items-center gap-1.5 self-start rounded-full border border-white/22 bg-[linear-gradient(135deg,rgba(15,95,92,0.95),rgba(34,211,238,0.95),rgba(236,72,153,0.95))] px-4 py-2 font-display text-xs font-medium uppercase tracking-[0.18em] dark:text-white text-gray-900 shadow-[0_14px_44px_rgba(0,0,0,0.5)] transition-transform hover:-translate-y-0.5 sm:self-auto sm:px-5"
        >
