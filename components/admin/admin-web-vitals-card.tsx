...
export function AdminWebVitalsCard({ summary }: { summary: WebVitalsDashboardSummary }) {
  ...
  return (
    <section className="rounded-2xl border border-border/60 bg-card/40 backdrop-blur">
      ...
      {!summary.configured ? (
        <p className="px-4 py-8 text-center font-display text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
          vitals 수집 비활성
        </p>
          ...
          {coreMetrics.map((m) => {
            ...
            return (
              <li key={m.name} className="bg-card/50 p-4">
                ...
                <p
                  className={[
                    "mt-2 font-display text-xs font-medium uppercase tracking-[0.14em]",
