      ...
      {result ? (
        <>
          ...
          <section className="rounded-2xl border border-border/60 bg-card/40 backdrop-blur">
            <h2 className="flex items-center gap-2 border-b border-border/60 px-4 py-3 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">
              <Link2 className="h-4 w-4 text-red-400/80" aria-hidden />
              [ Broken links — {result.broken.length} ]
            </h2>
            {result.broken.length === 0 ? (
              <p className="px-4 py-8 text-center font-mono text-[11px] text-emerald-400/90">
                깨진 링크 없음
              </p>
            ) : (
              <ul className="max-h-[420px] divide-y divide-border/50 overflow-y-auto">
                {result.broken.map((row) => (
                  <li key={row.url} className="px-4 py-3">
                    <p className="font-mono text-[10px] text-primary">{row.path}</p>
                    <a
                      href={row.url}
                      target="_blank"
          ...
          {result.redirected.length > 0 ? (
            <section className="rounded-2xl border border-border/60 bg-card/40 backdrop-blur">
              <h2 className="border-b border-border/60 px-4 py-3 font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">
              ...
              <ul className="max-h-64 divide-y divide-border/50 overflow-y-auto">
                {result.redirected.map((row) => (
                  <li key={row.url} className="px-4 py-3">
                    <p className="font-mono text-[10px]">{row.path}</p>
