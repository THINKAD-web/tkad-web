  ...
  return (
    <div className={cn(reportPanelShell, className)}>
      <div className="border-b border-[color:var(--cr-border-soft)] px-4 py-3">
        <p className="tkad-type-label text-[color:var(--cr-fg-subtle)]">
          [ {title} ]
        </p>
      </div>
    ...
    <section className="relative overflow-hidden border-b border-[color:var(--cr-border-soft)] px-6 py-8 sm:px-10 sm:py-10">
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-6">
        <div className="min-w-0 flex-1">
          <p className="tkad-type-label text-[color:var(--cr-accent)]">
            [ THINKAD · 싱커드 ]
          </p>
          <p className="mt-1 tkad-type-label text-[color:var(--cr-fg-subtle)]">
            {`// `}OOH 광고 게재 완료 보고서
          </p>
        ...
        <div className="shrink-0 text-right">
          <span className="tkad-neon-border inline-block rounded-2xl bg-[var(--cr-surface)] px-3 py-1.5 tkad-type-label text-[color:var(--cr-fg)] backdrop-blur">
            [ {status || "—"} ]
          </span>
          <p className="mt-2 font-display tkad-type-note tracking-[0.18em] text-[color:var(--cr-fg-muted)]">
...
export function SectionTitle({ children }: { children: ReactNode }) {
  return (
    <h2 className="mb-3 tkad-type-label text-[color:var(--cr-accent)]">
      [ {children} ]
    </h2>
  );
}
...
export function SectionNote({ children }: { children: ReactNode }) {
  return (
    <p className="mt-2 tkad-type-note leading-relaxed text-[color:var(--cr-fg-muted)]">
      {`// `}
      {children}
    </p>
  );
...
export function StatCard({
  ...
  return (
    <div className={cn("rounded-[18px] border p-3.5 backdrop-blur-md", shell, className)}>
      <p className="tkad-type-label text-[color:var(--cr-fg-muted)]">
        [ {label} ]
      </p>
      <p
        className={cn(
...
export function NeonBadge({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex rounded-full border border-[color:var(--cr-accent-border)] bg-[var(--cr-accent-surface)] px-2 py-0.5 tkad-type-label text-[color:var(--cr-accent)]">
      {children}
    </span>
  );
}
...
export function InfoCell({
  ...
  return (
    <div className={cn("tkad-glass-surface-soft rounded-[18px] border border-[color:var(--cr-border-soft)] bg-[var(--cr-surface)] p-3.5 backdrop-blur-md", className)}>
      ...
      <p
        className={cn(
          "mt-1.5 tkad-type-title text-[color:var(--cr-fg)]",
...
export function StatCardFootnote({
  ...
  return (
    <div className={cn("space-y-1", className)}>
      ...
      {footnote ? (
        <p className="px-1 font-display tkad-type-note text-[color:var(--cr-fg-dim)]">{`// `}{footnote}</p>
      ) : null}
    </div>
  );
}
    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
      {items.map((x, i) => (
        ...
        >
          ...
          />
          <span className="absolute left-0 top-0 rounded-br-[10px] border border-[color:var(--cr-border)] bg-[var(--cr-thumb-badge-bg)] px-2 py-1 tkad-type-label text-[color:var(--cr-accent)] backdrop-blur">
            [ {x.tag} ]
          </span>
        </div>
      ))}
...
export function NeonTable({
  ...
  return (
    <div className="overflow-x-auto rounded-[18px] border border-[color:var(--cr-border)] bg-[var(--cr-surface-soft)] backdrop-blur-sm">
      <table className="w-full min-w-[640px] border-collapse tkad-type-caption">
        <thead>
          <tr className="border-b border-[color:var(--cr-border-soft)] bg-[var(--cr-surface)]">
            {headers.map((h) => (
              <th
                key={h}
                className="px-3 py-2.5 text-left tkad-type-label text-[color:var(--cr-accent)]"
              >
                [ {h} ]
              </th>
            ))}
...
export function DistBar({
  ...
  return (
    <div className="rounded-[14px] border border-[color:var(--cr-border-soft)] bg-[var(--cr-surface)] p-3 backdrop-blur-sm">
      <div className="mb-2 flex justify-between gap-2 tkad-type-caption">
...
export function ReportFooter() {
  return (
    <footer className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t border-[color:var(--cr-border-soft)] pt-6">
      <div>
        <p className="tkad-type-label text-[color:var(--cr-fg-soft)]">
          [ THINKAD · 싱커드 ]
        </p>
        <p className="mt-1 tkad-type-note text-[color:var(--cr-fg-dim)]">
          {`// `}
          {CONTACT_EMAIL} · 02-515-2772 · 서울특별시 성동구
        </p>
      </div>
      <p className="font-display tkad-type-note text-[color:var(--cr-fg-dim)]">{`// `}© 2026 THINKAD</p>
    </footer>
  );
}
