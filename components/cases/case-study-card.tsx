    ...
    >
      <Link href={`/cases/${item.id}`} className="relative block overflow-hidden">
        <ContentCardGradientThumb
          ...
          badge={
            <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">
              {showAnonymizedBadge ? (
                <span className="rounded-full border border-gray-200 bg-white/90 px-2.5 py-0.5 tkad-type-label text-gray-700 shadow-sm dark:border-white/25 dark:bg-black/55 dark:text-white/90">
                  {t("badgeAnonymized")}
                </span>
              ) : null}
              <span className="rounded-full border border-[color:var(--qp-accent)]/35 bg-[color:var(--qp-accent-soft)] px-2.5 py-0.5 tkad-type-label text-[color:var(--qp-accent)]">
                {item.industry}
              </span>
              {region ? (
                <span className="inline-flex items-center gap-1 rounded-full border border-gray-200 bg-white/90 px-2.5 py-0.5 tkad-type-label text-gray-600 shadow-sm dark:border-white/15 dark:bg-black/50 dark:text-white/80">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-gray-900/25 opacity-0 transition-opacity duration-300 group-hover:opacity-100 dark:bg-black/50">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 tkad-type-label text-white backdrop-blur">
            {t("hoverViewDetails")}
      ...
      <div className={cn("flex flex-1 flex-col p-3 text-left", !compact && "p-4")}>
        ...
        <h3
          className={cn(
            "line-clamp-2 tkad-type-title leading-snug tracking-tight text-gray-900 dark:text-white",
        ...
        {!compact ? (
          <div className="mt-3 flex flex-wrap gap-1">
            {item.mediaUsed.slice(0, 3).map((m) => (
              <span
                key={m}
                className="rounded-full border border-gray-200 bg-gray-100 px-2 py-0.5 font-display tkad-type-note uppercase tracking-[0.12em] text-gray-600 dark:border-white/10 dark:bg-white/10 dark:text-white/70"
              >
                {m}
              </span>
            ))}
        ...
        {!compact ? (
          <Link
            ...
            className="mt-4 inline-flex items-center gap-1 tkad-type-label text-gray-500 transition-colors hover:text-[color:var(--qp-accent)] dark:text-white/60"
