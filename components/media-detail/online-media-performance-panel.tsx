...
export function OnlineMediaPerformancePanel({ media, locale, className }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {(bucket === "ko") ? "예상 성과 (참고)" : "Estimated performance (reference)"}
      </h2>
      ...
      {estimate ? (
        ...
        <p className="mt-4 text-sm text-gray-500 dark:text-white/55">
          {(bucket === "ko") ? "예산을 입력하면 범위를 보여줍니다." : "Enter a budget to see ranges."}
        </p>
      )}

      {estimate ? (
        <p className="mt-3 text-xs text-gray-500 dark:text-white/50">
          {(bucket === "ko") ? "근거:" : "Basis:"} {estimate.basis.join(" · ")}
        </p>
      ) : null}
