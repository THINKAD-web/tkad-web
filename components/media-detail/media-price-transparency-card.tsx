...
export function MediaPriceTransparencyCard({
  ...
  return (
    <section
      className="mt-6 rounded-2xl border-2 border-cyan-500/30 bg-gradient-to-br from-cyan-950/40 via-card to-violet-950/30 p-5 sm:p-6"
      aria-labelledby="price-transparency-heading"
    >
      <p
        id="price-transparency-heading"
        className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-700 dark:text-cyan-400/90"
      >
        {isKo ? "실시간 시세" : "Market rate"}
      </p>
      <h3 className="mt-2 text-lg font-black text-foreground">
        {isKo ? "가격 투명성" : "Price transparency"}
      </h3>
      ...
      <ul className="mt-4 space-y-3 text-sm text-foreground">
        <li className="flex flex-wrap items-baseline justify-between gap-2 border-b border-border/60 pb-3">
          <span className="text-muted-foreground">
            {isKo
              ? `${benchmark.zoneLabelKo} 평균 단가`
              : `${benchmark.zoneLabelEn} average`}
          </span>
        ...
        <li className="flex flex-wrap items-baseline justify-between gap-2">
          <span className="text-muted-foreground">
            {isKo ? "유사 매체 시세 범위" : "Similar media range"}
          </span>
      ...
      <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
        {isKo
          ? `협상 가능 폭은 보통 표시가의 ${benchmark.negotiationHintPct}% 전후입니다. 「가격 제안하기」로 매체사에 익명 제안할 수 있습니다.`
          : `Negotiation room is often around ${benchmark.negotiationHintPct}% off list. Use “Propose a price” to reach the owner anonymously.`}
      </p>

      {benchmark.zoneId ? (
        <RegionPriceAlertToggle
          regionZone={benchmark.zoneId}
          zoneLabel={isKo ? benchmark.zoneLabelKo : benchmark.zoneLabelEn}
          locale={locale}
        />
      ) : null}
    </section>
