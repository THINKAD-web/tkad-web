...
export default function MediaAiRecommendDashboard({
  ...
  return (
    <div className="space-y-8 rounded-2xl border border-gold/20 bg-navy p-4 shadow-xl sm:p-6 lg:p-8">
      <header className="space-y-1 text-center sm:text-left">
        ...
        <p className="text-sm text-slate-400">{tr("resultSubtitle")}</p>
      </header>

      <section className="space-y-3">
        <h3 className="text-sm font-bold text-gold">{tr("resultMapTitle")}</h3>
        <MediaAiRecommendMap
          locale={locale}
          items={mapItems}
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-bold text-gold">{tr("resultTop3Title")}</h3>
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-bold text-gold">{tr("resultChartTitle")}</h3>
      ...
      <div className="flex flex-col gap-3 border-t border-gold/15 pt-6 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        ...
        <Link href={quoteHref}>
          <Button
            type="button"
            size="lg"
            className="w-full rounded-full border border-gold bg-gold/10 font-bold text-gold hover:bg-gold/20 sm:w-auto"
          >
...
function Top3DashCard({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-xl border border-gold/25 bg-navy/50 p-4 shadow-inner",
        rankRingClass,
      )}
    >
      ...
      <dl className="mt-4 space-y-2 text-xs">
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">{tr("resultEstImpressions")}</dt>
          <dd className="tabular-nums font-semibold text-slate-200">
            {monthly > 0 ? monthly.toLocaleString() : "—"}
          </dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">{tr("resultCpmLabel")}</dt>
          <dd className="tabular-nums font-semibold text-slate-200">
        ...
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">{isKo ? "가격" : "Price"}</dt>
