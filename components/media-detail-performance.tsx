...
export default function MediaDetailPerformance({
  ...
  return (
    <section
      ...
      className="mt-12 border-2 border-bx-black bg-bx-white p-6 sm:p-8"
    >
      ...
      <div className="grid grid-cols-1 gap-0 sm:grid-cols-2">
        <div className="-mt-[2px] -ml-[2px] border-2 border-bx-black bg-bx-off px-6 py-7">
          <p className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-bx-gray-dim">
            [ {t("performanceStatMonthly")} ]
          </p>
          <p className="mt-3 text-3xl font-bold tabular-nums tracking-tight text-bx-black whitespace-nowrap sm:text-4xl">
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-2 lg:items-center">
        <div>
          <p className="mb-4 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-bx-gray-dim">
          ...
          <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-center sm:gap-8">
            ...
            >
              <div className="absolute inset-[24%] border-2 border-bx-black bg-bx-white" />
            </div>
            <ul className="w-full max-w-xs space-y-3 text-sm">
              {metrics.donut.map((seg) => (
                <li
                  key={seg.key}
                  className="flex items-center justify-between gap-3 border-b-2 border-bx-black pb-2 last:border-0 last:pb-0"
                >
                  <span className="flex items-center gap-2 text-bx-black">
                    <span
                  ...
                  <span className="font-mono text-sm font-bold tabular-nums text-bx-black">
                    {seg.percent}%
                  </span>
                </li>
              ))}
            ...
            {metrics.bars.map((row) => (
              <li key={row.key}>
                <div className="mb-1.5 flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="font-mono text-lg font-bold tabular-nums text-bx-black">
                    {row.value}
                  </span>
                </div>
                <div
                  className="h-3 w-full border-2 border-bx-black bg-bx-white"
                  role="progressbar"
                ...
                >
                  <div
                    className="h-full bg-bx-accent transition-[width] duration-500"
      ...
      <div className="mt-10 border-2 border-bx-black bg-bx-black px-6 py-7 text-bx-white sm:px-7 sm:py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {visibilityBadge ? (
            <span
              className={
                metrics.visibilityScore >= 90
                  ? "border-2 border-bx-accent bg-bx-accent px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-bx-white"
                  : "border-2 border-bx-white px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-bx-white"
              }
            >
              {visibilityBadge}
            </span>
        ...
        <div className="mt-4 flex flex-wrap items-end gap-2">
          <span className="text-5xl font-bold tabular-nums tracking-tight text-bx-accent sm:text-6xl">
            {metrics.visibilityScore}
          </span>
          <span className="mb-2 font-mono text-sm uppercase tracking-[0.18em] text-bx-white/60">
            / 100
          </span>
        </div>
        <div
          className="mt-4 h-4 w-full border-2 border-bx-white bg-bx-black"
          role="progressbar"
      ...
      <p className="mt-8 border-2 border-bx-black bg-bx-off px-5 py-4 text-sm font-medium leading-relaxed text-bx-black sm:px-6">
