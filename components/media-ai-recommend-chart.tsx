...
function ChartTooltip({
  ...
  return (
    <div className="rounded-2xl border border-border/80 bg-card/90 px-3 py-2 text-xs shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur">
      ...
      <p className="mt-1 tkad-type-caption tabular-nums text-muted-foreground">
        {isKo ? "추천 점수" : "Fit score"}: <span className="text-foreground">{p.x}</span>
      </p>
      <p className="tkad-type-caption tabular-nums text-muted-foreground">
        {isKo ? "월간 노출(추정)" : "Est. monthly reach"}:{" "}
        <span className="text-foreground">{p.y.toLocaleString()}</span>
      </p>
      {p.rank <= 3 ? (
        <p className="mt-1 tkad-type-label text-accent">
          [ TOP {p.rank} ]
        </p>
      ) : null}
    </div>
...
export default function MediaAiRecommendChart({ locale, scored }: Props) {
  ...
  if (!data.length || !hasReach) {
    return (
      <div
        className="flex min-h-[280px] items-center justify-center border-2 border-border bg-muted px-4 text-center tkad-type-label text-muted-foreground"
        role="img"
    ...
    >
      <div className="mb-2 flex flex-wrap items-end justify-between gap-2 px-1">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-2 rounded-full border border-border/80 bg-card/80 px-3 py-1 tkad-type-label text-muted-foreground shadow-xs backdrop-blur">
