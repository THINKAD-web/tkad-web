...
function MediaDetailQuoteModalBody({
  ...
  return (
    <div className="px-4 pb-5 pt-9 sm:px-6 sm:pb-6 sm:pt-10">
      <div className="mb-4 flex items-start gap-3 border-b-2 border-bx-black pb-4">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center border-2 border-bx-black bg-bx-accent text-bx-white">
          <Sparkles className="h-5 w-5" strokeWidth={1.75} aria-hidden />
        </span>
        <div className="min-w-0">
          <h2
            id="media-quote-modal-title"
            className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-bx-gray-dim"
          >
            [ {t("quoteModalTitle")} ]
          </h2>
          <p className="mt-1.5 text-base font-bold leading-snug text-bx-black sm:text-lg">
            {mediaTitle}
          </p>
        </div>
      </div>
      <p className="mb-4 text-sm leading-relaxed text-bx-black/80">
        {t("quoteModalDescription")}
      </p>

      {hasOpts ? (
        <div
          className="mb-5 border-2 border-bx-black bg-bx-off p-3 sm:p-4"
          role="radiogroup"
          aria-label={t("quoteModalPriceOptionLabel")}
        >
          <p className="mb-2 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-bx-gray-dim">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bx-accent/40 focus-visible:ring-offset-2",
                    selectedCard
                      ? "border-bx-accent bg-bx-white"
                      : "border-bx-black/15 bg-bx-white hover:border-bx-black/30",
                  )}
                >
                  <span
                    className={cn(
                      ...
                      selectedCard
                        ...
                        : "border-bx-black/20 bg-bx-white text-transparent",
                    )}
                    aria-hidden
                  >
                    <Check className="size-2.5 stroke-[4]" />
                  </span>
                  <span className="block pr-8 text-sm font-bold leading-snug text-bx-black">
                    {o.label}
                  </span>
                  <span className="mt-1.5 block text-lg font-bold tabular-nums tracking-tight text-bx-accent sm:text-xl">
                    {formatCatalogPriceKrwLong(o.price, locale)}
                  </span>
                  <span className="mt-1.5 font-mono text-[9px] font-bold uppercase tracking-wider text-bx-gray-dim">
                    {t(
                      mediaDetailPricePeriodTranslationKey(
                  ...
                  {o.description?.trim() ? (
                    <span className="mt-2 block border-t-2 border-bx-black/5 pt-2 text-[11px] leading-relaxed text-bx-black/70">
                      {o.description}
                    </span>
                  ) : null}
                </button>
          ...
          {selected ? (
            <p className="mt-3 border-t-2 border-bx-black/10 pt-3 text-sm text-bx-black">
              <span className="font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-bx-gray-dim">
        ...
        <p className="mb-5 border-2 border-bx-accent/40 bg-bx-off px-3 py-2.5 text-sm font-bold text-bx-black">
          {media.keywordFilter.priceText}
        </p>
      ) : (
        <div className="mb-5 border-2 border-bx-black bg-bx-white px-3 py-3">
          <p className="font-mono text-[9px] font-bold uppercase tracking-wider text-bx-gray-dim">
            {isKo ? "기준 요금" : "Base rate"}
          </p>
          <p className="mt-1.5 text-lg font-bold tabular-nums text-bx-accent">
            {formatCatalogPriceKrwLong(media.price, locale)}
          </p>
          <p className="mt-0.5 font-mono text-[9px] text-bx-gray-dim">
            {t(
...
export default function MediaDetailQuoteModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      className="max-w-lg border-2 border-bx-black bg-bx-white"
