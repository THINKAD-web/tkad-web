    ...
    <div className="space-y-3">
        ...
        {OPTIONS.map((option) => {
          ...
          return (
            <button
              ...
              disabled={isPending}
              className={[
                "inline-flex items-center justify-center border-2 px-3 py-2 tkad-type-label transition-colors disabled:cursor-not-allowed disabled:opacity-70",
                active
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className="inline-flex items-center justify-center border-2 border-border bg-foreground px-4 py-2 tkad-type-label text-background transition-colors hover:bg-primary hover:border-primary hover:text-primary-foreground disabled:cursor-not-allowed disabled:opacity-60"
        >
          {isPending ? "저장 중..." : "관리 정보 저장"}
        </button>
        {dirty && !isPending ? (
          <span className="tkad-type-note tracking-tight text-amber-700">
            {`// `}변경 사항 있음
          </span>
        ) : null}
      </div>
