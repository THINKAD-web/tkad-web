  ...
  const inner = (
    <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border-2 border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-[0_18px_60px_rgba(15,23,42,0.10)]">
      <div className="relative aspect-[16/10] w-full overflow-hidden border-b-2 border-border bg-muted">
        ...
        {item.usageCount > 0 ? (
          <span className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-md border border-accent bg-accent px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.18em] text-accent-foreground">
        ...
        {item.tags.length > 0 ? (
          <div className="mt-1 flex flex-wrap gap-1">
            {item.tags.slice(0, 4).map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center gap-0.5 rounded-md border border-border bg-muted px-1.5 py-0.5 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-foreground/80"
              >
