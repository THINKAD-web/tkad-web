      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <select
            ...
            className="h-11 w-full rounded-md border-2 border-border bg-card px-3 tkad-type-title text-foreground focus:border-accent focus:outline-none"
          >
            {KNOWN_TYPES.map((t) => (
        ...
        <div>
          ...
          {issues.length > 0 ? (
            <ul className="mt-3 space-y-1 rounded-lg border-2 border-amber-500 bg-amber-50 p-3 tkad-type-meta text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
              {issues.map((i) => (
                <li key={i}>· {i}</li>
              ))}
            </ul>
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map((t) => (
                  <button
                    ...
                    type="button"
                    onClick={() => removeTag(t)}
                    className="inline-flex items-center gap-1 rounded-md border-2 border-border bg-muted px-2 py-1 tkad-type-label text-foreground transition-colors hover:border-destructive hover:text-destructive"
                  >
                    #{t}
      ...
      <aside className="space-y-4">
        <div className="rounded-2xl border-2 border-border bg-muted/40 p-4 sm:p-5">
          ...
          <div className="mt-3 aspect-[16/10] w-full overflow-hidden rounded-2xl border-2 border-border dark:bg-black bg-white dark:bg-white/5 bg-gray-50 bg-white/30">
            {previewUrl ? (
              ...
              <div className="flex h-full w-full items-center justify-center tkad-type-label text-muted-foreground">
                파일을 선택하면 미리보기가 표시됩니다
              </div>
            )}
          </div>
          {meta ? (
            <dl className="mt-3 grid grid-cols-2 gap-2 tkad-type-label text-muted-foreground">
        ...
        {preselectedMedia ? (
          <div className="rounded-2xl border-2 border-accent bg-accent/10 p-4 sm:p-5">
            ...
            <div className="mt-3 inline-flex items-center gap-1 tkad-type-label text-accent">
...
function SpecPreview({ spec }: { spec: CreativeSpecRow }) {
  return (
    <div className="mt-3 rounded-xl border-2 border-border bg-card p-3 text-sm">
      ...
      <ul className="mt-2 space-y-1 tkad-type-meta text-foreground/85">
