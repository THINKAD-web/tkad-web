      ...
      <div className="tkad-landing-neon mx-auto max-w-lg px-4 py-10">
        <div className="tkad-glass-surface tkad-neon-border rounded-[28px] border p-6 sm:p-8">
          ...
          <div className="mt-6 rounded-2xl bg-muted/30 p-4">
            <p className="font-mono text-[10px] uppercase text-muted-foreground">{t("referralLinkLabel")}</p>
            <p className="mt-2 break-all text-sm font-medium">{data?.inviteLink}</p>
            <button
              type="button"
              onClick={() => void copyLink()}
              className="mt-3 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 font-mono text-xs font-bold uppercase text-primary-foreground"
            >
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-border/50 p-4 text-center">
              ...
              <p className="font-mono text-[10px] uppercase text-muted-foreground">{t("referralCount")}</p>
            </div>
            <div className="rounded-2xl border border-border/50 p-4 text-center">
              <p className="mt-2 text-2xl font-black tabular-nums text-primary">
                {(data?.totalEarned ?? 0).toLocaleString()}P
              </p>
              <p className="font-mono text-[10px] uppercase text-muted-foreground">{t("referralEarned")}</p>
            </div>
          </div>
