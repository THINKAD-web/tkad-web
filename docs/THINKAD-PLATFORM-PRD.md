# THINKAD Platform PRD — HOO 압도 하이브리드 플랫폼

> **Product Requirements Document v1.0**
> 작성일: 2026-04-18 · 작성자: THINKAD Product Team (AI-assisted)
> 대상 브랜치: `claude/write-thinkad-prd-MBid6`
> 기반 코드베이스: Next.js 16.2.3 / Prisma 7.6 / PostgreSQL / Anthropic SDK 0.88

---

## 목차

1. [비전 & 포지셔닝 전략](#1-비전--포지셔닝-전략)
2. [사용자 페르소나](#2-사용자-페르소나-3종)
3. [경쟁사 분석](#3-경쟁사-분석)
4. [기능별 상세 명세](#4-기능별-상세-명세)
5. [정보 구조(IA) 및 사용자 플로우](#5-정보-구조ia-및-사용자-플로우)
6. [추가 Prisma 스키마 설계](#6-추가-prisma-스키마-설계)
7. [API 엔드포인트 설계](#7-api-엔드포인트-설계)
8. [개발 로드맵](#8-개발-로드맵--phase별-스프린트)
9. [KPI & 성과 지표](#9-kpi--성과-지표)
10. [리스크 & 미티게이션](#10-리스크--미티게이션)
...
| 데이터 근거 | 매체사 자료만 | **공공데이터 유동인구 + Kakao POI + 자체 집행 이력** |
...
4. **One DB, Multi-facing** — Admin, Client, Public이 동일한 Prisma 모델을 공유한다 (`fetchPublicMediaCatalog`).

---

## 2. 사용자 페르소나 3종

### 2.1 Persona A — **박준호 (35)** · 스타트업 마케팅 리드

- **직무**: Series B SaaS 스타트업 Growth Lead, 연간 광고 예산 15억
- **상황**: 브랜드 인지도 확대를 위해 OOH 첫 집행 고려 중. 디지털 퍼포먼스 경험은 많지만 OOH는 낯섦.
- **목표(Jobs-to-be-Done)**:
  - 예산 3,000만원 내에서 서울 2030 타겟 매체 조합 찾기
  - 제안서를 내부 C레벨에 당일 공유
  - 집행 후 ROI 증빙
- **Pain Points**:
  - HOO에서 매체 고르면 "진짜 이 매체가 우리 타겟에 맞나?" 확신 부족
  - 유동인구 데이터 출처가 매체사마다 다름
  - 대행사 통하면 2주씩 걸리는 제안 대기
- **THINKAD 활용 시나리오**:
  1. `/planner` 7단계 위자드로 브랜드 목표 입력 → Claude가 매체 포트폴리오 추천
  2. 검증 배지(4/4) 매체 위주로 선별
  3. 제안서 PDF를 Slack에 공유 → 당일 승인
  4. `/client/dashboard`에서 집행 증빙 사진과 유동인구 리포트 확인
- **핵심 지표(북극성)**: 제안서 생성까지 걸리는 시간 (< 30분)

### 2.2 Persona B — **김미영 (42)** · 중견 대행사 AE

- **직무**: 종합광고대행사 매체본부 AE, 제약/금융 클라이언트 담당
- **상황**: 클라이언트별 연 5~10건 OOH 제안. 기존에는 OOH 본부 통해 수작업 견적.
- **목표(JTBD)**:
  - 클라이언트 브리핑 후 48시간 내 제안서 3안 제시
  - 전국 단위 멀티 시티 캠페인 설계
  - 매체 현황(가용성/가격) 실시간 확인
- **Pain Points**:
  - 내부 매체본부 응답 2~3일 소요
  - 경쟁 대행사도 같은 HOO를 보고 있어 차별화 어려움
  - 클라이언트 보고용 증빙 매 건마다 수작업
- **THINKAD 활용 시나리오**:
  1. **파트너 대행사 네트워크** 가입 → 수수료 셰어 구조로 전용 대시보드 사용
  2. `/admin/quote-templates` 기반 자사 브랜드 PDF 템플릿 사용
  3. `/compare` 툴로 매체 3개 옆에 두고 CPM/리치 비교 PDF 출력
  4. My THINKAD에서 지난 캠페인 성과 자동 리포트 → 클라이언트 재계약 설득
- **핵심 지표**: 월간 견적 생성 건수, 재계약율

### 2.3 Persona C — **이동철 (58)** · 지방 버스쉘터 매체사 대표

- **직무**: 부산/울산 지역 버스쉘터 240면 운영, 연매출 18억
- **상황**: 기존 영업망이 전화·엑셀 중심. 서울 광고주 접점 부족.
- **목표(JTBD)**:
  - 비수기 공실률 낮추기 (현재 35%)
  - 서울·수도권 광고주에게 노출
  - 집행 이력·가격 이력 관리 자동화
- **Pain Points**:
  - 온라인 노출 채널 없음
  - 매체사 자체 데이터(유동인구)가 없어 단가 협상력 약함
  - 집행 증빙 요청 올 때마다 직접 촬영 후 카톡 전송
- **THINKAD 활용 시나리오**:
  1. **미디어 오너 등록 포털** (Phase 3)에서 매체 240면 일괄 업로드
  2. 자동 지오코딩 + 공공데이터 유동인구 자동 삽입
  3. 캠페인 수주 시 현장 촬영을 PWA(Phase 4)로 즉시 업로드 → Proof Photo 자동 연결
  4. 월간 정산 리포트 자동 발행
...
| 항목 | **THINKAD (목표)** | HOO (House of OOH) | 전통 종합대행사 | 매체사 직거래 |
|---|---|---|---|---|
| **비즈니스 모델** | 하이브리드 (SaaS + 커미션 + 풀서비스) | 마켓플레이스 수수료 | 커미션 (15~20%) | 직거래 |
| **매체 수 (목표 YR1)** | 15,000+ (검증됨) | ~20,000 | 제한적 | 자사만 |
| **검증 시스템** | **4단계 배지** | 등록 심사만 | 수작업 | 없음 |
| **제안서 속도** | **< 30분 (AI)** | 24~48h | 3~7일 | 3~5일 |
| **가격 투명성** | **가격 이력 스냅샷 공개** | 호가만 | 비공개 | 협상 |
| **데이터 근거** | **공공유동인구 + 자체이력 + POI** | 매체사 자료 | 내부 DB | 없음 |
| **AI 추천** | **Claude 4.7 기반** | 없음 | 없음 | 없음 |
| **VR/AR 투어** | **Phase 3 제공** | 없음 | 일부 현장답사 | 현장답사 |
| **집행 증빙** | **필수, 자동 PDF** | 선택 | 사진 보고 | 요청 시 |
| **다국어** | **KO/EN/CN/JP** | KO | KO/EN | KO |
| **모바일 앱** | **PWA (Phase 4)** | 모바일 웹 | 없음 | 없음 |
| **가격 범위** | 50만~10억 | 50만~5억 | 1억+ | 협상 |

### 3.2 SWOT 분석

**Strengths**
- 현장 4단계 검증 역량 (기존 오프라인 자산)
- 풀스택 기술팀 + AI 통합 (Claude API)
- 기존 Prisma 데이터 모델 성숙도 (16 모델)
...
**Opportunities**
- OOH 시장 디지털 전환 초기 (DX 선점)
- 공공데이터 개방 가속 (유동인구·소상공인·교통)
- 글로벌 광고주 한국 OOH 수요 증가

**Threats**
- HOO의 자본력/네트워크 효과
- 전통 대행사의 반발 및 끼워팔기
- AI 환각에 의한 잘못된 추천 리스크
...
THINKAD는 **우상단 (High Tech × High Touch)** 의 빈 사분면을 선점한다.

---

## 4. 기능별 상세 명세

> 각 기능은 **[User Story] → [Acceptance Criteria] → [UI Guide] → [기술 구현]** 순서로 기술한다.
> 기존 구현(`현재`) / 신규 추가(`신규`) 태그로 구분.

### 4.1 Phase 1 (0–3개월) 핵심 기능
...
- 우측 65%: Kakao Map (primary color `#0D1B2E` 핀, 검증 매체는 `#C8913C` gold 테두리)
- 클러스터 색상: 매체 수 기준 (1~5 네이비, 6~20 앰버, 21+ 진한 앰버)
- 핀 hover: 작은 카드 (썸네일·가격), 클릭: 전체 사이드 카드

**기술 구현**
- 클라이언트: `components/media-map-view.tsx` 신규 생성, Kakao Maps JS SDK (`//dapi.kakao.com/v2/maps/sdk.js?appkey=<KEY>&libraries=clusterer,services`)
- 기존 `fetchPublicMediaCatalog()` 확장 → `bounds: { sw, ne }` 파라미터 추가
- API: `GET /api/public/media-catalog` 에 `boundingBox`, `radiusKm`, `centerLat`, `centerLng` 쿼리 지원
- Prisma: `Media.latitude`, `Media.longitude` 컬럼에 `@@index([latitude, longitude])` 공간 인덱스 추가 (추후 PostGIS 이관 가능성 열어둠)
- 지도 SSR 이슈 방지: `next/dynamic` 로 `ssr: false`
- 성능: Intersection Observer로 뷰포트 밖 매체 카드 언마운트
...
- 커버 페이지: 좌상단 THINKAD 로고, 우상단 QR코드 (제안서 URL), 중앙 "Verified" 도장

**기술 구현**
- Prisma 스키마에 `MediaVerificationBadge` 모델 신규 (아래 §6 참조)
...
- jsPDF에서 SVG 아이콘 렌더링은 `svg2pdf.js` 또는 base64 PNG pre-render
...
**기술 구현**
- Auth: `next-auth` v5 (현재 미설치, 신규) 또는 자체 세션 (`AdminUser` 모델 구조 재사용)
- Prisma 신규: `User`, `UserSession`, `UserFavoriteMedia`, `PlannerPlan`, `Notification` (§6)
...
- 알림 드롭다운: 최신 5개 + "모두 보기"
- 토스트: 페이지 우상단에서 슬라이드 인

**기술 구현**
- Prisma: `Notification`, `NotificationSubscription` (§6)
...
- 실시간성: 초기에는 폴링(30s), Phase 3에서 SSE 또는 Pusher 검토

---

### 4.2 Phase 2 (3–6개월) 확장 기능
...
- 입력 프롬프트에 지난달 Prisma 집계 쿼리 결과 주입 (prompt caching 적용 — Anthropic `cache_control`)
- 월 1일 03:00 cron: `/api/cron/generate-monthly-trend-report`
- `Subscriber` 모델 신규 (§6) — 이메일·관심 카테고리·언어
...
- [ ] 한국어/영어 자동 감지
- [ ] Rate limit: 로그인 사용자 60 req/hr, 게스트 10 req/hr
...
**기술 구현**
- `app/api/chat/route.ts` (기존 존재) 확장
- Anthropic SDK streaming + tool use
- Prompt caching: 시스템 프롬프트 + 매체 카탈로그 요약을 `cache_control: { type: 'ephemeral' }` 로 표시
- Tool handler: Prisma 쿼리로 실시간 매체 검색
- `ChatSession`, `ChatMessage` 모델 신규 (§6)
- Vercel Edge runtime (`export const runtime = 'edge'`) 또는 Node.js (Prisma 제약 확인)
...
- 렌더러: `marzipano` (pure JS, 15KB gzip) 또는 `@react-three/fiber` + `@react-three/drei`
- `MediaVRTour` 모델 (§6) — imageUrl, hotspotsJson, capturedAt
...
**Acceptance Criteria**
- [ ] 집행 증빙 사진(`CampaignProofPhoto`) 자동 수집 및 분류 (시점별·매체별)
- [ ] 유동인구 기반 추정 노출수 산출
- [ ] 업종 평균 대비 CPM 비교 그래프
...
**기술 구현**
- `MediaOwner`, `MediaOwnerMembership` 모델 (§6)
- `Media.ownerId` FK 추가
- CSV 파싱: `papaparse`
...
**기술 구현**
- `next-pwa` 또는 직접 service worker 작성 (Next.js 16 호환성 확인)
- IndexedDB로 오프라인 데이터 저장
...
- **Primary (상단 GNB)**: 매체 / 플래너 / 견적 / 비교 / 인사이트 / 사례 / 아카데미
- **Secondary (유저 메뉴)**: 알림벨 / My / 언어 / 로그인
- **Footer**: 회사 / 서비스 / 리소스 / 문의 / SNS

### 5.4 상태 파이프라인 (OoHQuote)
...
> 기존 16개 모델은 유지. 신규 추가분만 기술. `schema.prisma` 말미에 append 가정.
...
// =============================================
// Phase 2: Subscribers (Trend Report)
// =============================================
...
// Media 모델에 추가될 필드 (기존 모델 수정):
// ownerId String?
// owner   MediaOwner? @relation("MediaOwner", fields: [ownerId], references: [id])

// =============================================
// Phase 3: Agency Partner Network
// =============================================
...
- `QuoteTemplate`: `agencyId String?` + relation `@relation("AgencyTemplates")`
...
> 기존 89개 엔드포인트는 유지. 신규/확장 엔드포인트만 기술.
...
| Method | Path | 설명 |
|---|---|---|
| GET | `/api/public/media-catalog?bbox=&radiusKm=&centerLat=&centerLng=` | 지도 뷰 확장 쿼리 |
...
- **Rate limit**: Upstash Redis or DB-backed, 기존 `lib/rate-limit.ts` 확장
- **CSRF**: SameSite 쿠키 + Origin 헤더 검증
- **OpenAPI**: Phase 2 에 `openapi.yaml` 자동 생성 (zod → OpenAPI)
...
### 8.2 Phase 1 스프린트 (12주 / 6 스프린트)

- **Sprint 1 (W1-2)** · 인증 기반
  - `User`, `UserSession`, OAuth 스캐폴드
  - `/my` 라우트 트리 + 미들웨어 보호
  - 마이그레이션 · 시드 스크립트
- **Sprint 2 (W3-4)** · 지도 MVP
  - Kakao Map 통합, 카탈로그 bbox 쿼리
  - 핀 클러스터링 · 사이드 카드
- **Sprint 3 (W5-6)** · My THINKAD
  - 플래너 localStorage → 서버 동기화
  - 즐겨찾기·견적 이력·캠페인 탭
- **Sprint 4 (W7-8)** · 검증 배지 (읽기 전용)
  - `MediaVerificationBadge` 모델·시드
  - 카탈로그·상세·PDF 배지 노출
- **Sprint 5 (W9-10)** · 알림 시스템
  - Notification·Subscription 모델
  - Web Push · 이메일 디스패처
  - 알림 센터 UI
- **Sprint 6 (W11-12)** · 안정화
...
- Preview deploy: PR마다 Vercel/커스텀 런너에서 자동 배포
...
- Feature flag: `UNLEASH` 또는 간단한 `FeatureFlag` 테이블 (Phase 2에 도입)
...
- **에러율** < 0.5% (5xx)
- **빌드 성공률** ≥ 98%
- **E2E 테스트 커버리지** (핵심 플로우) ≥ 90%
- **Lighthouse Performance** ≥ 90 (모바일)

### 9.4 비즈니스 지표

- **GMV** (총거래액): Phase 1 말 월 10억 → Phase 4 월 50억+
- **Take rate**: 8~12%
- **CAC (고객 획득비)** < ₩150,000
- **LTV/CAC** > 4
...
| R3 | **AI 환각** → 잘못된 매체/가격 추천 | 높음 | 중 | tool use 강제, 응답 전 DB 재확인, 사용자에게 "확인 필요" 라벨 |
| R4 | **공공데이터 API 중단/지연** | 중 | 중 | 자체 heuristic 폴백 유지, 매주 데이터 덤프 캐싱 |
| R5 | **개인정보보호법/ PIPA 위반** | 치명 | 중 | 가입/로그인 flow에 약관·동의·마케팅 수신 분리, 데이터 내보내기/삭제 API, 암호화 저장 |
| R6 | **매체사 온보딩 느림** → 재고 부족 | 높음 | 높음 | 초기에 영업팀 매뉴얼 온보딩, 인센티브(3개월 수수료 면제), 자동 지오코딩 |
| R7 | **Kakao Map API 쿼터/과금** | 중 | 중 | 무료 한도 초과 시 Naver Map 백업, 뷰포트 debounce |
| R8 | **대행사 반발** (자체 채널 우회 유도) | 중 | 높음 | 파트너 네트워크로 인센티브 제공, 화이트라벨 제공 |
| R9 | **브라우저 호환성** (PDF/지도/VR) | 저 | 중 | Chrome/Safari/Edge 최신 2버전 지원 명시, E2E에 포함 |
| R10 | **SEO 경쟁** (HOO 선점 키워드) | 중 | 높음 | 콘텐츠 허브(인사이트/사례/아카데미), 구조화 데이터(schema.org Place) |
| R11 | **보안 사고** (서명 PDF 유출, 세션 탈취) | 치명 | 저 | 서명 PDF 암호화, 접근 로그, 세션 IP 바인딩, SOC2 로드맵 |
| R12 | **Next.js 16 breaking change** | 중 | 중 | `node_modules/next/dist/docs/` 의무 확인 (AGENTS.md), 주요 버전 업 시 스프린트 별도 할당 |
| R13 | **모바일 사용자 이탈** | 중 | 높음 | 모바일 우선 디자인, Phase 1부터 지도 바텀시트·플래너 세로 스크롤 최적화 |
| R14 | **Prisma/PostgreSQL 성능** (매체 1만+·지리쿼리) | 중 | 중 | 공간 인덱스·PostGIS 이관 플랜 (Phase 3), 읽기 전용 replica |
| R15 | **번역 품질** (AI 번역 부자연) | 중 | 중 | 전문 번역사 검수 루프, 주요 카피는 사람 확정 |

### 10.1 비상 계획 (Contingency)

- **서비스 장애**: Vercel → Railway/Fly.io 이중화 검토 (Phase 3)
- **데이터 유실**: 일 1회 PITR 백업, 주 1회 외부 S3 덤프
- **핵심 인력 이탈**: Runbook·ADR 문서화, Pair programming

### 10.2 법률/컴플라이언스 체크리스트
...
- Primary Navy: `#0D1B2E` (딥 잉크)
- Accent Gold: `#C8913C` (리치 앰버)
- Neutral: `#B0B8C4` / `#D6D9E6`
