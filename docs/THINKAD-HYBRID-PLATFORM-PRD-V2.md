...
- [ ] *(조건부)* 크리에이티브 시뮬레이터 v1 (이미지만) — **Sprint 4 말 F1.2 p95<60s + 보안 P0 4항목 통과 시에만 착수**, 미충족 시 Phase 2 Sprint 8로 이관 (§7.1 제언 1 참조)
...
### 6.3 3개월 OKR (Phase 1 MVP · 2026-05 ~ 2026-08 초, 13주)

#### O1 · "MVP 출시하여 사용자 손에 쥐어준다"

| KR | 측정 | 목표 | 기준선 | 담당 |
|---|---|---|---|---|
| KR1.1 | **베타 정식 출시**일 (지도·제안서·플래너·회원) | **2026-08-07** | — | PM |
...
| KR2.6 | 제안서 생성 시간 | **p95 < 60초, p50 < 45초** | 3일 (Before) | BE |
| KR2.7 | 공개 웹뷰 보안 이상 건수 (토큰 유출·악성 접근) | 0건 | — | BE/보안 |
...
### 6.4 6개월 OKR (Phase 2 · 2026-08 중 ~ 2026-11 초)
...
#### O8 · "챗봇 2단계 고도화 — Rule MVP로 시작해 Claude 정밀화"

**Stage 1 — Rule-based MVP (Sprint 13-14, W26-29)**

| KR | 측정 | 목표 |
|---|---|---|
| KR8.1a | 챗봇 일 활성 세션 (DAU) | **200건** (초기 목표, rule 기반) |
| KR8.2a | 누적 질문 로그 수집량 (Stage 2 프롬프트 재료) | **3,000건+** (Sprint 21 착수 전까지) |
| KR8.3a | "전문가 연결" 전환율 (하이브리드 핵심) | **10%+** |
| KR8.4a | FAQ 규칙 커버리지 (규칙으로 답변 완결된 비율) | 50%+ |

**Stage 2 — Claude 4.7 tool use 전환 (Sprint 21-22, W42-45)**

| KR | 측정 | 목표 |
|---|---|---|
| KR8.1b | 챗봇 일 활성 세션 (DAU) | **500건** |
| KR8.2b | 챗봇 → 제안서 전환율 | **15%** |
| KR8.3b | "전문가 연결" 전환율 (Stage 1 대비 유지 이상) | ≥ 10% |
| KR8.4b | 챗봇 Claude 평균 비용 / 세션 | **< $0.05** (prompt cache ON) |
| KR8.5b | Anthropic 월 예산 대비 사용률 | < 80% (초과 시 자동 rule-only 폴백) |
| KR8.6b | Stage 2 응답 만족도 (A/B vs Stage 1 rule) | 만족도 유의미 상승 |
...
| 시나리오 | 배치 | 트리거 조건 (Sprint 4 말 점검) | 범위 |
|---|---|---|---|
| **기본 (권장)** | **Phase 1 Sprint 5 (W10-11) 포함** | F1.2 PDF 파이프라인이 Sprint 4 말(W9)까지 **p95 < 60s 안정화 + 보안 AC 4개 통과** | 시뮬레이터 v1 (이미지만, 영상 제외) + 플래너 결과·PDF 자동 삽입 |
| **대체 (안전망)** | **Phase 2 초 Sprint 8 (W16-17)로 이관** | Sprint 4 말 위 조건 미충족 시 | 대신 Sprint 5는 F1.2 보안·안정화·Verified Only·3-Tier에 집중 |

**판단 기준 (Sprint 4 리뷰 미팅에서 PM 결정)**
- F1.2 기본 PDF 생성 p95 지표가 60s 이내로 들어왔는가
- 공개 웹뷰 보안 8항목 중 P0 4개(토큰 TTL·IP rate-limit·audit log·noindex)가 통과 상태인가
- FE 팀 여유: Sprint 5 크리에이티브 업로드·Cloudinary 합성 구현 4일 이상 확보 가능한가

**대체 시 득실 요약**
- **득**: F1.2 안정성 리스크 제거, Sprint 5가 "보안·배지·필터" 핵심에 집중
- **실**: 베타 30명이 "크리에이티브 미리보기 없음" 상태로 첫 구매 경험 → 결제 전환율 +15~25%p 기회 2~3주 지연
- **완화**: Phase 2 Sprint 8에서 유동인구 UI와 함께 착수 시 시너지 (매체 상세 페이지에 미리보기·유동인구·VR 3-tab 동시 출시)
