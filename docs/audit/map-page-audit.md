...
#### F. Production CARTO dark basemap 확인 (2026-09-23 KST ~18:15)

**운영:** Vercel **Production** `NEXT_PUBLIC_CARTO_BASEMAPS_API_KEY` 등록 · 재배포 `dpl_3sbg7JJ1WFoCL6Xkb77wj6q3J3ze` (`tkad-b8i1m89sz…`, ~18:04 KST). 코드 참조명 **`NEXT_PUBLIC_CARTO_BASEMAPS_API_KEY`** ↔ `getCartoBasemapsApiKey()` (`lib/public-dark-map-config.ts`) **일치**. 다크 경로: `publicMapTileUrlForTheme('dark')` → `resolvePublicRasterBasemapUrl(PUBLIC_MAP_TILE_URLS.dark)`.

| 확인 | 방법 | 결과 |
|------|------|------|
| **다크 타일 호스트** | Playwright headless — `localStorage` manual **dark** + `html.dark` | **`basemaps.cartocdn.com/dark_all/…png?key=…`** 12 req · **OSM 0** · VWorld 0 |
| **Lab LCP 타일** | `scripts/lighthouse-map-runs.mjs` ×3 · [`tkad.co.kr/ko/media/map`](https://tkad.co.kr/ko/media/map) · 측정 시각 **≥18:00 KST** (auto dark) | LCP **`img.leaflet-tile`** → **`c.basemaps.cartocdn.com/dark_all/…@2x.png?key=…`** (3/3) · median LCP **~26.0 s** (lab만; §E field SSOT) |

**결론:** Production 다크 basemap은 **OSM 폴백 아님** — **CARTO `dark_all` + API key** 정상. (이전 §C·§D 시점 Carto Production **없음** → 다크 OSM 폴백이었던 상태와 구분.)
